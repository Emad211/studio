    ...
    <Link href={href} className="group block h-full">
      <Card className={cn(
        ...
        <CardContent className="flex-grow p-6">
          <div className={cn("flex flex-wrap items-center gap-2 mb-3", isFa && "justify-end")}>
              {categories.map(category => (
                <Badge key={category} variant="secondary" className="font-normal text-xs">{category}</Badge>
              ))}
          </div>
